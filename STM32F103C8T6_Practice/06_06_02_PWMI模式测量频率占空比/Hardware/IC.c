...
void IC_Init(void)
{
	...
	//设置占空比捕获，采用同一个高级定时器的通道2捕获占空比
	tTIM_ICInitTypeDef.TIM_Channel = TIM_Channel_2;
	tTIM_ICInitTypeDef.TIM_ICFilter = 0xF;							//滤波
	tTIM_ICInitTypeDef.TIM_ICPolarity = TIM_ICPolarity_Falling;		//上升沿触发
	tTIM_ICInitTypeDef.TIM_ICPrescaler = TIM_ICPSC_DIV1;			//1分频
	tTIM_ICInitTypeDef.TIM_ICSelection = TIM_ICSelection_IndirectTI;//通道交叉连接
...
unsigned short IC_GetDuty(void)
{
