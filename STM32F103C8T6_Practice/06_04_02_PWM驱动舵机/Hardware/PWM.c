...
void PWM_Init(void)
{
	...
	tGPIO_InitTypeDef.GPIO_Pin = GPIO_Pin_1;
	...
	tTIM_TimeBaseInitTypeDef.TIM_Period = 20000 - 1;				
	...
	TIM_OC2Init(TIM2, &tTIM_OCInitTypeDef);							//初始化第1个通道，该通道输出默认复用GPIO的PA0端口
...
void PWM_SetCompare2(unsigned int _Compare)
{
	TIM_SetCompare2(TIM2, _Compare);
}
