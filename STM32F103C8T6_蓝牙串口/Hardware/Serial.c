...
unsigned char g_uchRxData[128] = { 0 };
...
void Serial_GetRxData(unsigned char** _ppAddr, unsigned short* _pSize)
{
	if(_ppAddr == NULL || _pSize == NULL)
		return;
	
	*_ppAddr = g_uchRxData;
	*_pSize = strlen((const char*)g_uchRxData);
}
...
{
	static char chBranchFlag = 0;				//状态机分支
	static unsigned short usRxPos = 0;			//数据存储位置
	unsigned char uchRxData = 0;				//当前读取到的数据
	
	if(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == SET)
	{
		uchRxData = USART_ReceiveData(USART1);
		...
		switch (chBranchFlag)
		{
			...
			case 1:
			{
				if(uchRxData == ']')
				{
					g_uchRxData[usRxPos] = 0;		//防止读取到无效的数据
...
void Serial_SendPacket(unsigned char* _pAddr, unsigned short _Size)
{
	unsigned short i = 0;
