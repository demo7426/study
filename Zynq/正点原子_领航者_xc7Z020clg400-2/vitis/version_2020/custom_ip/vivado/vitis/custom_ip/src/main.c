...
int main()
{
	...
	while(1)
	{
		//根据nFreqFlag的标志位，切换呼吸灯的频率（频率范围为:0x1~0xa）
		if(nFreqFlag == 0)
		{
		...
		if(nLedState == 0)
		{
			BREATH_LED_IP_mWriteReg(LED_IP_BASEADDR, LED_IP_REG0, 1);		//打开LED
			print("Breath LED ON\r\n");
			sleep(5);
		}

		nLedState = BREATH_LED_IP_mReadReg(LED_IP_BASEADDR, LED_IP_REG0);	//关闭LED
		if(nLedState == 1)
		{
