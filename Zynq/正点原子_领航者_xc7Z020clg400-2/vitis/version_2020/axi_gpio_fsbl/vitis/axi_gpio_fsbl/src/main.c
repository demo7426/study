...
#define _DIRRECT_CONNECT 			1								//是否直连的标志;0--非直连;1--直连;
...
int main()
{
	...
	XGpio_SetDataDirection(&tXGpio, GPIO_CHANNEL1, 1);
	...
	while(1)
	{
		...
		if(!XGpio_DiscreteRead(&tXGpio, GPIO_CHANNEL1))		//读取通道对应的离散状态，实现按键的输入波形为下降沿时触发
...
#endif
		//使能中断
		XGpio_InterruptEnable(&tXGpio, GPIO_CHANNEL1);
...
static void IntrHandler(void *CallBackRef)
{
	...
	XGpio *ptGpio = CallBackRef;

	if(XGpio_InterruptGetStatus(ptGpio))
		g_uchKeyPress = 1;

	//取消中断使能
	XGpio_InterruptDisable(ptGpio, GPIO_CHANNEL1);
