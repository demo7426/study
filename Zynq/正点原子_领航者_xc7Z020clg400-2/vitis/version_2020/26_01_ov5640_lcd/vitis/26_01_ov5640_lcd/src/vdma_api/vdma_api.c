...
static int StartTransfer(XAxiVdma *InstancePtr, enum VDMA_OPERATE_TYPE _OpType);
...
int run_vdma_frame_buffer(XAxiVdma* InstancePtr, int DeviceId, int hsize,
		...
		int enable_frm_cnt_intr, enum VDMA_OPERATE_TYPE _OpType)
{
	int Status,i;
	...
	if(_OpType == ONLY_WRITE || _OpType == READ_WRITE)
	{
	...
	if(_OpType == ONLY_READ || _OpType == READ_WRITE)
	{
	...
	Status = StartTransfer(vdma_context[DeviceId].InstancePtr, _OpType);
	if (Status != XST_SUCCESS) {
		if(Status == XST_VDMA_MISMATCH_ERROR)
			xil_printf("DMA Mismatch Error\r\n");
		return XST_FAILURE;
