...
//定时器中断初始化
static int Timet_Intr_Init(XGpioPs* _pXGpioPs, XScuTimer *_pXScuTimer, XScuGic *_pXScuGic);
...
int main()
{
	...
	XScuTimer_Start(&tXScuTimer);		//启动定时器
...
static int Timet_Intr_Init(XGpioPs* _pXGpioPs, XScuTimer *_pXScuTimer, XScuGic *_pXScuGic)
{
	...
	nStatus = XScuGic_Connect(_pXScuGic, TIMER_INTR_ID,
	...
	//使能定时器中断
	XScuTimer_EnableInterrupt(_pXScuTimer);
...
static void Timer_Intr_Handler(void* _pArg)
{
	...
	XGpioPs* ptXGpioPs = (XGpioPs*)_pArg;

	XGpioPs_WritePin(ptXGpioPs, MIO_LED, ~XGpioPs_ReadPin(ptXGpioPs, MIO_LED));		//反转LED亮灭效果
}
