...
struct CHAR_DEV{
    ...
    atomic_t KeyStatus;                                         //按键当前状态

    wait_queue_head_t WaitQueHead;                              //读等待队列头

    struct fasync_struct* pAsync_Queue;                         //fasync_struct结构体
};
...
/// @param _pWait 
/// @return 
__poll_t Key_Poll(struct file* _pFp, struct poll_table_struct* _pWait);  

static int Key_FAsync(int _Fd, struct file* _pFp, int _On)
{
    return fasync_helper(_Fd, _pFp, _On, &g_tCharDev.pAsync_Queue);
}
...
static void Timer_CallBack(struct timer_list* _pTimer)
{
    ...
    {
        atomic_set(&g_tCharDev.KeyStatus, KEY_PRESS);
        wake_up_interruptible(&g_tCharDev.WaitQueHead);                     //唤醒等待队列头中所有队列

        if(g_tCharDev.pAsync_Queue)
            kill_fasync(&g_tCharDev.pAsync_Queue, SIGIO, POLL_IN);          //发出信号，通知用户态程序
    }
    else if(nLastKeyState == 0 && nCurKeyState == 1)
    {
        atomic_set(&g_tCharDev.KeyStatus, KEY_RELEASE);
        wake_up_interruptible(&g_tCharDev.WaitQueHead);                     //唤醒等待队列头中所有队列
        ...
        atomic_set(&g_tCharDev.KeyStatus, KEY_KEEP);
...
static int __init Key_Init(void)
{
    ...
    //初始化等待队列头
    init_waitqueue_head(&g_tCharDev.WaitQueHead);
...
ssize_t Key_Read(struct file* _pFp, char __user* _pBuf, size_t _Len, loff_t* _pOffset)
{
    ...
    if(_pFp == NULL || _pBuf == NULL || _Len < sizeof(nCurStatus))
    {
        printk(KERN_ALERT "Key_Read input param err, %p, %p, %d.\n", _pFp, _pBuf, _Len);
        return -EFAULT;
    }

    if(_pFp->f_flags & O_NONBLOCK)          //非阻塞方式访问
    {
        if(atomic_read(&g_tCharDev.KeyStatus) == KEY_KEEP)
            return -EAGAIN;
    }
    else                                    //阻塞方式访问
    {
        //加入等待队列，当有按键按下或释放时，才会被唤醒
        nRet = wait_event_interruptible(g_tCharDev.WaitQueHead, atomic_read(&g_tCharDev.KeyStatus) != KEY_KEEP);
        if(nRet)
            return nRet;
    }

    nCurStatus = atomic_read(&g_tCharDev.KeyStatus);

    if (copy_to_user(_pBuf, &nCurStatus, sizeof(nCurStatus)))                                //将内核数据拷贝到用户空间
    {
...
__poll_t Key_Poll(struct file* _pFp, struct poll_table_struct* _pWait)
{
    ...
    poll_wait(_pFp, &g_tCharDev.WaitQueHead, _pWait);

    if(atomic_read(&g_tCharDev.KeyStatus) != KEY_KEEP)      //按键按下或释放
