...
修改记录:
  ...
  1.  日期: 2025.09.09
      作者: 钱锐
      内容:
          1) 扩展新增信号量，保证该字符设备，在未关闭时，app只能打开1次；
      版本:V0.1.1
...
static int __init Led_Init(void)
{
    ...
    sema_init(&g_tCharDev.Sem, 1);                  //初始化信号量
...
int Led_Open(struct inode* _pInode, struct file* _pFp)
{
    ...
    if(down_interruptible(&g_tCharDev.Sem))                                 //获取信号量，和 down 类似，只是使用down 进入休眠状态的线程不能被信号打断。而使用此函数进入休眠以后是可以被信号打断的。
    {   
        //获取信号失败
