...
修改记录:
  ...
  1.  日期: 2025.09.09
      作者: 钱锐
      内容:
          1) 扩展新增原子量，保证该字符设备，在未关闭时，app只能打开1次；
      版本:V0.1.1
...
static int __init Led_Init(void)
{
    ...
    atomic_set(&g_tCharDev.AtomicLock, 1);      //对原子量进行赋值
...
int Led_Open(struct inode* _pInode, struct file* _pFp)
{
    if(atomic_dec_return(&g_tCharDev.AtomicLock) != 0)              //已经被使用
    {
        printk(KERN_ALERT "Led_Open is busy.\n");
        atomic_inc(&g_tCharDev.AtomicLock);
        return -EBUSY;
    }
