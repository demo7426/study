...
int CXDeCode::Set_AVCodecParameters(AVCodecParameters* _pAVCodecParameters)
{
	std::unique_lock<std::mutex> lock(m_mut);

	int nRtn = avcodec_parameters_to_context(m_ptAVCodecContext, _pAVCodecParameters);		//解封装的视频编码参数，传递给解码上下文
	if (nRtn < 0)
	{
		PrintErr(nRtn);
		return EXIT_FAILURE;
