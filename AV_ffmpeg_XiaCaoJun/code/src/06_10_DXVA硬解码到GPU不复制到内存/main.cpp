/*************************************************
Copyright (C), 2009-2012    , Level Chip Co., Ltd.
文件名:	main.cpp
作  者:	钱锐      版本: V0.1.0     新建日期: 2025.08.15
描  述: 实现一个h264、h265的解码，并渲染到ui上
备  注: ffmpeg -i xxx.mp4 -s 400x300 06_05_ffmpeg软解码.h264
修改记录:

  1.  日期: 2025.08.15
	  作者: 钱锐
	  内容:
		  1) 此为模板第一个版本;
	  版本:V0.1.0

*************************************************/

#include <iostream>
#include <string>
#include <regex>
#include <fstream>
#include <chrono>
#include <thread>

#include "xsdl.h"
#include "xsdl.h"
#include "d3d9.h"

extern "C" {
#include <libavcodec/avcodec.h>
#include <libavutil/opt.h>
}

#pragma comment(lib, "avcodec.lib")
#pragma comment(lib, "avutil.lib")

struct DXVA2DevicePriv
{
	HMODULE d3dlib;
	HMODULE dxva2lib;

	HANDLE device_handle;

	IDirect3D9* d3d9;
	IDirect3DDevice9* d3d9device;
};

/// <summary>
/// GPU直接渲染视频图像
/// </summary>
/// <param name="_pAVFrame"></param>
/// <param name="_pAVCodecContext"></param>
/// <returns></returns>
static int GPU_DrawFrame(AVFrame* _pAVFrame, AVCodecContext* _pAVCodecContext)
{
	if (!_pAVFrame || !_pAVFrame->data[3] || !_pAVCodecContext)
	{
		std::cout << __FUNCTION__ << " input paramter is err." << std::endl;
		return -2;
	}

	auto surface = (IDirect3DSurface9*)_pAVFrame->data[3];
	auto ctx = (AVHWDeviceContext*)_pAVCodecContext->hw_device_ctx->data;
	auto priv = (DXVA2DevicePriv*)ctx->user_opaque;
	auto device = priv->d3d9device;

	static RECT viewport;
	static HWND hwnd = nullptr;

	if (!hwnd)
	{
		hwnd = CreateWindow(L"DXVA", L"Test DXVA2", WS_OVERLAPPEDWINDOW,
			0, 0,
			_pAVFrame->width, _pAVFrame->height,
			0, 0, 0, 0);

		ShowWindow(hwnd, SW_NORMAL);
		UpdateWindow(hwnd);

		viewport.left = 0;
		viewport.right = _pAVFrame->width;
		viewport.top = 0;
		viewport.bottom = _pAVFrame->height;
	}

	//设置窗口句柄
	device->Present(&viewport, &viewport, hwnd, 0);

	//后台缓冲表面
	static IDirect3DSurface9* back = nullptr;
	if (!back)
		device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &back);

	device->StretchRect(surface, 0, back, &viewport, D3DTEXF_LINEAR);
}

int main()
{
	int nRtn = 0;													//返回值
	std::string strSrcFilePath = "06_05_ffmpeg软解码.h264";			//源文件路径
	AVCodecID eAVCodecID = AV_CODEC_ID_NONE;						//格式ID

	std::regex pattern(R"(\.([^.]+)$)");							//匹配最后一个点及其后的内容
	std::smatch match;

	//创建windows窗口
	WNDCLASSEX tWndClassEx = { 0 };
	tWndClassEx.cbSize = sizeof tWndClassEx;
	tWndClassEx.lpfnWndProc = DefWindowProc;						//消息函数
	tWndClassEx.lpszClassName = L"DXVA";
	RegisterClassEx(&tWndClassEx);

	if (std::regex_search(strSrcFilePath, match, pattern)) {
		// match[1] 包含第一个捕获组，即后缀部分
		if (strcmp(match[1].str().c_str(), "h264") == 0)
			eAVCodecID = AV_CODEC_ID_H264;
		else if (strcmp(match[1].str().c_str(), "h265") == 0)
			eAVCodecID = AV_CODEC_ID_H265;
		else
		{
			std::cout << "源文件：" << strSrcFilePath << " 格式错误" << std::endl;
			return EXIT_FAILURE;
		}
	}
	else {
		std::cout << "未找到后缀" << std::endl;
	}

	//解码并渲染

	AVCodec* ptAVCodec = nullptr;
	AVCodecContext* ptAVCodecContext = nullptr;
	AVCodecParserContext* ptAVCodecParserContext = nullptr;
	AVPacket* ptAVPacket = nullptr;
	AVFrame* ptAVFrame = nullptr;								//CPU解码后的原始数据
	AVFrame* ptAVFrame_Hw = nullptr;							//GPU解码后的原始数据
	AVFrame* ptAVFrame_Tmp = nullptr;							//解码后的临时数据		

	CXVideo_View* m_pcXVideo_View = nullptr;
	CXVideo_View::Format eFmt = CXVideo_View::Format::YUV420P;

	// 记录开始时间点
	auto cStart_TimePoint = std::chrono::high_resolution_clock::now();
	auto cEnd_TimePoint = cStart_TimePoint;

	char chReadBuf[4096] = { 0 };			//读取缓冲
	int nReadValidLen = 0;					//读取有效长度
	int nAV_ParserLen = 0;					//取出AV_Packet有效长度

	int nFrameCounter = 0;					//帧数计数

	std::fstream ifs(strSrcFilePath, std::ios::binary | std::ios::in);
	if (!ifs.is_open())
	{
		std::cout << "源文件打开失败" << std::endl;
		return EXIT_FAILURE;
	}

	//找到解码器 
	ptAVCodec = avcodec_find_decoder(eAVCodecID);
	if (!ptAVCodec)
	{
		std::cerr << "avcodec_find_decoder is err" << std::endl;
		return EXIT_FAILURE;
	}

	//解码器上下文
	ptAVCodecContext = avcodec_alloc_context3(ptAVCodec);
	if (!ptAVCodecContext)
	{
		std::cerr << "avcodec_alloc_context3 is err" << std::endl;
		return EXIT_FAILURE;
	}

#if 1
	//硬件加速
	
	for(int i = 0;;i++)
	{
		auto config = avcodec_get_hw_config(ptAVCodec, i);
		if (!config)
			break;

		if (config->device_type)
			std::cout << av_hwdevice_get_type_name(config->device_type) << std::endl;
	}

	AVHWDeviceType eAVHwDeviceType = AV_HWDEVICE_TYPE_DXVA2;							//硬件加速格式 DXVA2

	//硬件加速上下文
	AVBufferRef* ptAVBufferRef = nullptr;
	av_hwdevice_ctx_create(&ptAVBufferRef, eAVHwDeviceType, NULL, NULL, 0);

	ptAVCodecContext->hw_device_ctx = av_buffer_ref(ptAVBufferRef);
#endif

	//ptAVCodecContext->thread_count = std::thread::hardware_concurrency();
	ptAVCodecContext->thread_count = 1;

	nRtn = avcodec_open2(ptAVCodecContext, NULL, NULL);
	if (nRtn != 0)
	{
		char chErrBuf[1024] = { 0 };
		av_strerror(nRtn, chErrBuf, sizeof(chErrBuf) - 1);

		std::cerr << "avcodec_open2 is err" << std::endl;
		std::cout << chErrBuf << std::endl;
		return EXIT_FAILURE;
	}

	ptAVPacket = av_packet_alloc();
	if (!ptAVPacket)
	{
		std::cerr << "av_packet_alloc is err" << std::endl;
		return EXIT_FAILURE;
	}

	ptAVFrame = av_frame_alloc();
	ptAVFrame_Hw = av_frame_alloc();

	ptAVCodecParserContext = av_parser_init(static_cast<int>(eAVCodecID));			//初始化解码器

	m_pcXVideo_View = CXVideo_View::Create();

	while (!ifs.eof())		//循环读取源文件数据，并解码
	{
		ifs.read(chReadBuf, sizeof(chReadBuf));

		nReadValidLen = ifs.gcount();
		auto chReadBufTmp = chReadBuf;

		while (nReadValidLen > 0) {
			nAV_ParserLen = av_parser_parse2(
				ptAVCodecParserContext, ptAVCodecContext,
				&(ptAVPacket->data), &(ptAVPacket->size),							//输出数据
				reinterpret_cast<const uint8_t*>(chReadBufTmp), nReadValidLen,		//输入数据
				AV_NOPTS_VALUE, AV_NOPTS_VALUE, 0
			);

			chReadBufTmp += nAV_ParserLen;
			nReadValidLen -= nAV_ParserLen;

			if (ptAVPacket->size == 0)
				continue;

			//std::cout << ptAVPacket->size << " ";

			//解码
			//发送编码后的数据到线程中解码
			nRtn = avcodec_send_packet(ptAVCodecContext, ptAVPacket);
			av_packet_unref(ptAVPacket);   // ★ 重要：释放引用，避免残留
			if (nRtn)
			{
				break;
			}

			while (nRtn >= 0)		//返回多帧
			{
				static bool bInitFlag = false;

				//接受压缩帧，一遍前几次调用返回空（缓冲，立即返回，编码未完成）
				//编码狮子啊独立的线程中
				//每次调用会重新分配ptAVPacket中的空间
				nRtn = avcodec_receive_frame(ptAVCodecContext, ptAVFrame);
				if (nRtn == AVERROR(EAGAIN) || nRtn == AVERROR_EOF)
				{
					break;
				}

				if (nRtn < 0)		//异常
				{
					char chErrBuf[1024] = { 0 };
					av_strerror(nRtn, chErrBuf, sizeof(chErrBuf) - 1);

					std::cerr << "av_frame_get_buffer is err" << std::endl;
					std::cout << chErrBuf << std::endl;
					return EXIT_FAILURE;
				}

				ptAVFrame_Tmp = ptAVFrame;

				//if (ptAVCodecContext->hw_device_ctx)		//硬解码
				//{
				//	//硬解码转换 GPU->CPU（显存->内存）
				//	//AV_PIX_FMT_NV12

				//	av_hwframe_transfer_data(ptAVFrame_Hw, ptAVFrame, 0);
				//	
				//	av_frame_unref(ptAVFrame);

				//	ptAVFrame_Tmp = ptAVFrame_Hw;
				//}

				if (!bInitFlag)
				{
					switch (ptAVFrame_Tmp->format)
					{
					case AVPixelFormat::AV_PIX_FMT_RGBA:
						eFmt = CXVideo_View::Format::RGBA;
						break;
					case AVPixelFormat::AV_PIX_FMT_ARGB:
						eFmt = CXVideo_View::Format::ARGB;
						break;
					case AVPixelFormat::AV_PIX_FMT_YUV420P:
						eFmt = CXVideo_View::Format::YUV420P;
						break;
					case AVPixelFormat::AV_PIX_FMT_NV12:
						eFmt = CXVideo_View::Format::NV12;
						break;
					default:
						break;
					}

					//m_pcXVideo_View->Init(ptAVFrame_Tmp->width, ptAVFrame_Tmp->height, eFmt, nullptr);
					bInitFlag = true;
				}

				//m_pcXVideo_View->DrawFrame(ptAVFrame_Tmp);
				GPU_DrawFrame(ptAVFrame_Tmp, ptAVCodecContext);
				av_frame_unref(ptAVFrame_Tmp);
				
				nFrameCounter++;

				cEnd_TimePoint = std::chrono::high_resolution_clock::now();
				if (std::chrono::duration_cast<std::chrono::milliseconds>(cEnd_TimePoint - cStart_TimePoint).count() >= 1000)
				{
					std::cout << "每秒帧数：" << nFrameCounter << std::endl;

					nFrameCounter = 0;
					cStart_TimePoint = cEnd_TimePoint;
				}

			}
		}

	}

	//处理解码线程中的残留数据
	nRtn = avcodec_send_packet(ptAVCodecContext, NULL);
	while (nRtn >= 0)		//返回多帧
	{
		//接受压缩帧，一遍前几次调用返回空（缓冲，立即返回，编码未完成）
		//编码狮子啊独立的线程中
		//每次调用会重新分配ptAVPacket中的空间
		nRtn = avcodec_receive_frame(ptAVCodecContext, ptAVFrame);
		if (nRtn == AVERROR(EAGAIN) || nRtn == AVERROR_EOF)
		{
			break;
		}

		if (nRtn < 0)		//异常
		{
			char chErrBuf[1024] = { 0 };
			av_strerror(nRtn, chErrBuf, sizeof(chErrBuf) - 1);

			std::cerr << "av_frame_get_buffer is err" << std::endl;
			std::cout << chErrBuf << std::endl;
			return EXIT_FAILURE;
		}

		ptAVFrame_Tmp = ptAVFrame;
		//if (ptAVCodecContext->hw_device_ctx)		//硬解码
		//{
		//	硬解码转换 GPU->CPU（显存->内存）
		//	AV_PIX_FMT_NV12

		//	av_hwframe_transfer_data(ptAVFrame_Hw, ptAVFrame, 0);

		//	ptAVFrame_Tmp = ptAVFrame_Hw;
		//}

		//m_pcXVideo_View->DrawFrame(ptAVFrame_Tmp);
		GPU_DrawFrame(ptAVFrame_Tmp, ptAVCodecContext);

		nFrameCounter++;

		cEnd_TimePoint = std::chrono::high_resolution_clock::now();
		if (std::chrono::duration_cast<std::chrono::milliseconds>(cEnd_TimePoint - cStart_TimePoint).count() >= 1000)
		{
			std::cout << "每秒帧数：" << nFrameCounter << std::endl;

			nFrameCounter = 0;
			cStart_TimePoint = cEnd_TimePoint;
		}
	}

	//清理资源
	if (ifs.is_open())
		ifs.close();

	av_parser_close(ptAVCodecParserContext);
	ptAVCodecParserContext = nullptr;

	av_packet_free(&ptAVPacket);
	ptAVPacket = nullptr;

	av_frame_free(&ptAVFrame);
	ptAVFrame = nullptr;

	av_frame_free(&ptAVFrame_Hw);
	ptAVFrame_Hw = nullptr;

	avcodec_free_context(&ptAVCodecContext);
	ptAVCodecContext = nullptr;

	// 新增：窗口消息循环，必须添加！
	MSG msg;
	while (GetMessage(&msg, NULL, 0, 0)) {
		TranslateMessage(&msg);
		DispatchMessage(&msg);
	}

	return EXIT_SUCCESS;
}
