...
文件名:	main.cpp
作  者:	钱锐      版本: V0.1.0     新建日期: 2026.03.11
描  述: 音视频同步推流
备  注:
修改记录:
...
static int AudioRecord_PushStream(int argc, char* argv[])
{
	...
	if (cSMux.Init(strDesUrl.data()))
	{
		qDebug() << __func__ << ", init failed";
		return -2;
	}

	if (cSMux.AddStream(cSAudioEncode.GetAVCodecContext()))
	{
		qDebug() << __func__ << ", init failed";
		return -2;
	}
...
static int VideoRecord_PushStream(int argc, char* argv[])
{
	...
	try
	{
		...
		if (cSMux.AddStream(cSVideoEncode.GetAVCodecContext()))
		{
			qDebug() << __func__ << ", init failed";
			return -2;
		}
...
/// <param name="argv"></param>
/// <returns></returns>
static int AVRecord_PushStream(int argc, char* argv[])
{
	int nRtn = 0;
	//std::string strSrcUrl = "rtsp://admin:level@12@10.0.0.93:554/Stream";
	std::string strSrcUrl = "D:/BaiduNetdiskDownload/mksz279-2022年经典再升级-FFmpeg5.0核心技术精讲，打造音视频播放器【完结】/{1}--第1章学习指南【课程提供200+问题与答案库】/[1.1]--1-2FFmpeg5.0-课程导学【瑞客论坛 www.ruike1.com】.mp4";
	...
	if (cSVideoEncode.Init(CSEncode::SAVCodecID::H264))
	{
		qDebug() << __func__ << ", CSEncode init is err";
		return -2;
	}
	...
	if (!ptSwsContext)
	{
		qDebug() << __func__ << ", sws_getCachedContext is err";
		return -2;
	}

	if (cSVideoEncode.Open(nSrcW, nSrcH, nSrcFps, CSEncode::SAVPixelFormat::YUV420P))
	{
		qDebug() << __func__ << ", CSEncode Open is err";
		return -2;
	}

	if (cSMux.Init(strDesUrl.data()))
	{
		qDebug() << __func__ << ", CSMux init is err";
		return -2;
	}
	...
	if (cSMux.Write_Header())
	{
		qDebug() << __func__ << ", CSMux Write_Header is err";
		return -2;
	}
	...
	std::thread thVideo([&]() {
		...
		while (g_bRunning)
		{
			...
			pcSFilter->Filter(&cMat, &cMatProc);			//过滤

			uchInData[0] = cMatProc.data;
			nInSize[0] = cMatProc.cols * cMatProc.elemSize();
			nInHeight = cMatProc.rows;

			//输出的AVFrame结构
			ptAVFrame_Video->width = cMatProc.cols;
			ptAVFrame_Video->height = cMatProc.rows;
			ptAVFrame_Video->format = AVPixelFormat::AV_PIX_FMT_YUV420P;

			//分配空间
			int nRet = av_frame_get_buffer(ptAVFrame_Video,
				32										//如果为0，则使用默认的32字节对齐
			);
			if (nRet)
			{
				av_frame_free(&ptAVFrame_Video);
				ptAVFrame_Video = nullptr;
			...
			if (!ptAVPacket)		//发送未压缩数据到线程中压缩
			{
				av_frame_unref(ptAVFrame_Video);
				continue;
			}

			std::cout << "v：size=" << ptAVPacket->size << ", pts=" << ptAVPacket->pts << "\n";
...
int main(int argc, char* argv[])
{
	...
	//return VideoRecord_PushStream(argc, argv);
	return AVRecord_PushStream(argc, argv);
}
