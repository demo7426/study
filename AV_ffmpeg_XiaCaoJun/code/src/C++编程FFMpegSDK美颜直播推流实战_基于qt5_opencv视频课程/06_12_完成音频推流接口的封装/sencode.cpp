...
int CSVideoEncode::Open(int _Width, int _Height, int _FrameRate, SAVPixelFormat _PixFmt)
{
	std::unique_lock<std::mutex> lock(m_mut);
...
int CSAudioEncode::Open(int _SampleRate, int _Channels)
{
	m_ptAVCodecContext->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
	m_ptAVCodecContext->thread_count = 1;
	m_ptAVCodecContext->bit_rate = 128000;
	m_ptAVCodecContext->sample_rate = _SampleRate;
	m_ptAVCodecContext->sample_fmt = AV_SAMPLE_FMT_FLTP;
	m_ptAVCodecContext->channels = _Channels;
	m_ptAVCodecContext->channel_layout = av_get_default_channel_layout(_Channels);
