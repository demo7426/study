...
namespace _301集_数据的初始化和LINQ的第一个基础使用
{
    internal class Program
    {
        static void Test01()
        {
            ...
            var masterList02 = from m in masterList
                               join k in kongfuList on m.KungFuName equals k.Name
                               where k.Lethality >= 100
                               select new { master = m, kungFu = k };//上下两种select写法均可以
                               //select new { m, k };

            //var masterList02 = masterList.SelectMany(m => kongfuList, (m, k) => new { master = m, kungFu = k })
            //    .Where(x => x.master.KungFuName == x.kungFu.Name);
