...
awaitable<void> CSession::Start(void)
{
	try
	{
		...
		while (true)
		{
			co_await m_cSocket.async_read_some(boost::asio::buffer(m_chRecvBuf, m_uchTLVType_OccupyBytes), boost::asio::use_awaitable);
			ushTLV_Type = *(unsigned short*)(m_chRecvBuf);

			co_await m_cSocket.async_read_some(boost::asio::buffer(m_chRecvBuf + m_uchTLVType_OccupyBytes, m_uchTLVLen_OccupyBytes), boost::asio::use_awaitable);
			ushShouldRecvLen = *(unsigned short*)(m_chRecvBuf + m_uchTLVType_OccupyBytes);

			co_await m_cSocket.async_read_some(boost::asio::buffer(m_chRecvBuf + uchTLVHead_OccupyBytes, ushShouldRecvLen), boost::asio::use_awaitable);

			CLogicSystem::GetInstance()->PushBack_RecvMsg_Node(
				std::make_shared<CRecvMsg_Node>(this->shared_from_this(), ushTLV_Type, m_chRecvBuf + uchTLVHead_OccupyBytes, ushShouldRecvLen)
			);
		}
	}
	catch (const std::exception& e)
	{
		std::cout << "Start is err, " << e.what() << std::endl;
...
void CSession::Send(char* _pMsg, int _MsgLen)
{
	...
	m_queSend.push(std::make_shared<CSendMsg_Node>(this->shared_from_this(), _pMsg, _MsgLen));
