...
CServer::~CServer()
{
	CAsioIOServeicePool::GetInstance()->Stop();
}
...
{
	boost::asio::io_context& cIO_Context = CAsioIOServeicePool::GetInstance()->GetIOContext();

	std::shared_ptr<CSession> pcSession = std::make_shared<CSession>(cIO_Context, this);
