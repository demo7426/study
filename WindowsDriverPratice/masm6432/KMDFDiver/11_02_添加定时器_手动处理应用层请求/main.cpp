...
描  述: 测试添加定时器、手动队列，在定时器中取出手动队列的请求,然后处理应用层请求
备  注:
修改记录:
...
int main()
{
    ...
    if (!DeviceIoControl(hDevice, IO_NUMCONVER_CODE, &chInNum, sizeof chInNum, pchOutNum, sizeof pchOutNum, &dwRet, NULL))
    {
