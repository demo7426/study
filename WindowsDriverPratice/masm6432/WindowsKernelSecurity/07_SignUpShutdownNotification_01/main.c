...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
    ...
    lNTStatus = IoCreateDevice(DriverObject, 0, NULL, FILE_DEVICE_UNKNOWN, 0, FALSE, &ptDevObj);
    if (!NT_SUCCESS(lNTStatus))
    {
        KdPrint(("创建设备文件失败\n"));
        return lNTStatus;
	...
	IoRegisterShutdownNotification(ptDevObj);							//对指定的设备注册关机通知
