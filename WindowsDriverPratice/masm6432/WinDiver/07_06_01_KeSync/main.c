...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
	...
	ptDeviceObject->Flags |= DO_BUFFERED_IO;					//设置缓冲的操作方式
...
NTSTATUS Dispatch_BufferIOControl(_In_ struct _DEVICE_OBJECT* DeviceObject, _Inout_ struct _IRP* Irp)
{
	...
	PCHAR pchInBuf = Irp->AssociatedIrp.SystemBuffer;									//输入buf
	PCHAR pchOutBuf = Irp->AssociatedIrp.SystemBuffer;									//输出buf
	...
	__try
	{
		...
		break;
		case IO_EVENT_CODE:				//事件同步
		{
			HANDLE handEvent = *(PHANDLE)pchInBuf;
			PKEVENT ptKEvent;

			KdPrint(("IO_EVENT_CODE\n"));

			//因为事件是从用户态传下来的对象，必须使用 UserMode 显示指明
			lNTStatus = ObReferenceObjectByHandle(handEvent, EVENT_MODIFY_STATE, *ExEventObjectType, UserMode, &ptKEvent, NULL);
			if (NT_SUCCESS(lNTStatus))
			{
				KeSetEvent(ptKEvent, IO_NO_INCREMENT, FALSE);
				ObDereferenceObject(ptKEvent);

				KdPrint(("IO_EVENT_CODE: KeSetEvent\n"));
			}
