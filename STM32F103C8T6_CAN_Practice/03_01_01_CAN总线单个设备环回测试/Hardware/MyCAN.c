...
void MyCAN_Transmit(uint32_t _ID, uint8_t* _pData, uint8_t _Length)
{
	...
	tCanTxMsg.StdId = _ID;
	tCanTxMsg.ExtId = _ID;
	tCanTxMsg.IDE = CAN_Id_Standard;
	tCanTxMsg.RTR = CAN_RTR_Data;			//帧类型
	tCanTxMsg.DLC = _Length;					//数据载荷长度
	
	memcpy(tCanTxMsg.Data, _pData, _Length);
	
	CAN_Transmit(CAN1, &tCanTxMsg);
}
...
void MyCAN_Receive(uint32_t* _pID, uint8_t _Data[8], uint8_t* _pLength)
{
	...
	if(!_pID || !_Data || !_pLength)
		return;
	
	CAN_Receive(CAN1, CAN_FIFO0, &tCanRxMsg);
	
	if(tCanRxMsg.IDE == CAN_Id_Standard)	//标准id
	{
		*_pID = tCanRxMsg.StdId;
	}
	else
	{
		*_pID = tCanRxMsg.ExtId;
	}
	
	if(tCanRxMsg.RTR == CAN_RTR_Data)			//数据帧
	{
		*_pLength = tCanRxMsg.DLC;
		memcpy(_Data, tCanRxMsg.Data, tCanRxMsg.DLC);
	}
	else																	//遥控帧
	{
		//...
