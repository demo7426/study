...
描  述: CAN总线数据传输策略：定时传输、触发传输、请求传输
备  注:  
修改记录: 
...
uint32_t g_unTxMsgIndex = 0;

//发送信息
CanTxMsg g_tCanTxMsg[] = {
	//StdId 	ExtId       IDE              RTR           DLC	Data[8]
	{0x555, 0x00000000, CAN_Id_Standard, CAN_RTR_Data, 		4, 	{ 0x00, 0x01, 0x02, 0x03}},
	...
	{0x000, 0x14785236, CAN_Id_Extended, CAN_RTR_Remote, 	0, 	{ 0x00, 0x00, 0x00, 0x00}},
};
...
int main(void)
{
	...
	CanTxMsg tCanTxMsg = { 0x111, 0x00000000, CAN_Id_Standard, CAN_RTR_Data, 		4, 	{ 0xaa, 0xbb, 0xcc, 0xdd} };
	...
	while(1)
	{
		...
		if(uchKeyNum == 1)
		{
			MyCAN_Transmit(&g_tCanTxMsg[g_unTxMsgIndex]);	//触发传输
			
			g_tCanTxMsg[g_unTxMsgIndex].Data[0]++;
			g_tCanTxMsg[g_unTxMsgIndex].Data[1]++;
			g_tCanTxMsg[g_unTxMsgIndex].Data[2]++;
			g_tCanTxMsg[g_unTxMsgIndex].Data[3]++;
			
			g_unTxMsgIndex++;
			g_unTxMsgIndex = g_unTxMsgIndex % (sizeof(g_tCanTxMsg) / sizeof(g_tCanTxMsg[0]));
		}
...
void TIM2_IRQHandler(void)
{
	...
	{
		MyCAN_Transmit(&g_tCanTxMsg[g_unTxMsgIndex]);		//定时传输
